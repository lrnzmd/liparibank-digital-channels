import { Request, Response } from "express";
import { UUID } from "../../types/common";
import { CustomerService } from "./customer.service";

/**
 * Controller per gli endpoint Customer.
 * Responsabilità:
 * - Estrarre dati da req (params, body)
 * - Chiamare il service
 * - Formattare la risposta HTTP con envelope { data: ... } e status code
 * - Gestire errori HTTP (404, 400, ecc.)
 */
export class CustomerController {
  constructor(private readonly service: CustomerService) {}

  /**
   * GET /api/customers
   * Recupera tutti i customer.
   */
  async getAll(req: Request, res: Response): Promise<void> {
    try {
      const customers = await this.service.getAll();
      res.status(200).json({ data: customers });
    } catch (error) {
      res.status(500).json({ error: "Failed to retrieve customers" });
    }
  }

  /**
   * GET /api/customers/:id
   * Recupera un customer per ID.
   */
  async getById(req: Request, res: Response): Promise<void> {
    try {
      const id = req.params.id as UUID;
      const customer = await this.service.getById(id);
      res.status(200).json({ data: customer });
    } catch (error) {
      res.status(404).json({ error: "Customer not found" });
    }
  }

  /**
   * POST /api/customers
   * Crea un nuovo customer.
   */
  async create(req: Request, res: Response): Promise<void> {
    try {
      const customer = await this.service.create(req.body);
      res.status(201).json({ data: customer });
    } catch (error) {
      res.status(400).json({ error: "Failed to create customer" });
    }
  }

  /**
   * PUT /api/customers/:id
   * Aggiorna un customer.
   */
  async update(req: Request, res: Response): Promise<void> {
    try {
      const id = req.params.id as UUID;
      const customer = await this.service.update(id, req.body);
      res.status(200).json({ data: customer });
    } catch (error) {
      res.status(404).json({ error: "Customer not found" });
    }
  }

  /**
   * DELETE /api/customers/:id
   * Elimina un customer.
   */
  async delete(req: Request, res: Response): Promise<void> {
    try {
      const id = req.params.id as UUID;
      const deleted = await this.service.delete(id);
      if (!deleted) {
        res.status(404).json({ error: "Customer not found" });
        return;
      }
      res.status(204).send();
    } catch (error) {
      res.status(500).json({ error: "Failed to delete customer" });
    }
  }
}
